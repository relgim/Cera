"""Direct provider-free integration audit for correction cycle 005.

The runner selects the exact v5 ingress, final-candidate, accepted-context,
authority-identity, canary, and subprocess-observer tests. It constructs no
live provider transport and opens only a disposable read-only SQLite copy.
"""
...
TASK_ID = "continuous-corrections-v5-provider-free-integration-audit"

CASES = (
    (
        1,
        "Explicit ingress ownership replaces text heuristics",
        "tests.test_continuous_corrections.ContinuousAuthorityV5Tests.test_ingress_owned_source_units_replace_text_heuristics",
    ),
    (
        2,
        "Continuous ingress source coverage is gap-free",
        "tests.test_continuous_corrections.ContinuousAuthorityV5Tests.test_continuous_request_rejects_unclassified_source_gaps",
    ),
    (
        3,
        "Composer exact claims and realization segments agree",
        "tests.test_continuous_corrections.ContinuousAuthorityV5Tests.test_composer_must_annotate_every_exact_protected_source_occurrence",
    ),
    (
        4,
        "Composer paraphrased and hidden Ted ownership fails",
        "tests.test_continuous_corrections.ContinuousAuthorityV5Tests.test_composer_rejects_paraphrased_or_hidden_ted_ownership",
    ),
    (
        5,
        "Final, event, and edit cannot extend a valid Ted claim",
        "tests.test_continuous_corrections.ContinuousAuthorityV5Tests.test_final_event_and_edit_cannot_extend_one_valid_ted_claim",
    ),
    (
        6,
        "Candidate authority manifest is immutable",
    ...
    (
        7,
        "Final actors, subjects, segments, and edits remain traceable",
        "tests.test_continuous_corrections.ContinuousEvidenceCorrectionTests.test_final_sequence_and_edits_retain_beat_traceability",
    ),
    (
        8,
        "Public, owner-private, and ownerless authority projections fail closed",
        "tests.test_continuous_corrections.ContinuousAuthorityV5Tests.test_accepted_projection_rejects_public_or_cross_owner_private_state",
    ),
    (
        9,
        "Pre-v5 continuous session compatibility cannot resume",
        "tests.test_continuous_planner_validator.ContinuousSessionLifecycleTests.test_restart_rejects_pre_v5_policy_compatibility",
    ),
    (
        10,
        "New canary identity is parameterized and historical identity is rejected",
        "tests.test_continuous_job4_harness.ContinuousJob4HarnessTests.test_parameterized_canary_identity_rejects_historical_reuse",
    ),
    (
        11,
        "Exact JobHarness completes all ten fake stages",
        "tests.test_continuous_job4_harness.ContinuousJob4HarnessTests.test_exact_job_harness_completes_all_ten_provider_free_stages",
    ),
    (
        12,
        "Actual subprocess sidecar marks thread_run failure",
        "tests.test_continuous_job4_harness.ContinuousJob4HarnessTests.test_actual_subprocess_sidecar_marks_post_submit_failure",
    ),
    (
        13,
...
def sqlite_check(source: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="cera-corrections-v5-job4-") as directory:
...
def main() -> int:
    ...
    payload = {
        ...
        "verification": [
            {
                "command": "direct provider-free corrections-v5 audit",
