...
The readable schemas under ``schemas/pi_scene/ordinary_review/v2`` are the
...
PYTHON_TARGET = ROOT / "src" / "cera" / "generated" / "ordinary_review_contracts_v2.py"
JAVASCRIPT_TARGET = (
...
DOC_TARGET = ROOT / "docs" / "generated" / "ORDINARY_REVIEW_CONTRACTS_V2.md"
...
def _materialize_detached_decision_hashes(
    ...
    decision_schema = next(
        schema
        for _, schema, _ in loaded
        if schema.get("x-cera-schema-version") == DECISION_VERSION
    )
        ...
        if isinstance(current, dict):
            ...
            if set(current) == {"$cera-example-ref"}:
                ...
                try:
                    ...
                    raise ValueError(
                        f"negative ordinary example is invalid: {marker}"
                    ) from exc
                patch = reference.get("patch")
                if patch is not None:
                    selected = _apply_patch(selected, patch)
...
def _validate_review_mode_controls(review: dict[str, object]) -> None:
    ...
    if controls["schema_version"] != "cera.pi_scene.request_controls.v3":
        _review_error("ordinary review v2 requires request-controls v3")
...
def _validate_decision(value: dict[str, object]) -> None:
    ...
    if action in {"automatic_accept", "accept", "accept_provisional"}:
...
def _validate_review_invariants(schema_version: str, value: dict[str, object]) -> None:
    ...
    if schema_version == "cera.pi_scene.review_checks.v1":
        return
    if schema_version == "cera.pi_scene.review.v2":
        _validate_review(value)
        return
    if schema_version == "cera.pi_scene.review_lifecycle.v1":
        _validate_initial_lifecycle(value)
        return
    if schema_version == "cera.pi_scene.review_decision.v2":
...
function validateReviewModeControls(review) {
  ...
  if (controls.schema_version !== 'cera.pi_scene.request_controls.v3') {
    reviewError('ordinary review v2 requires request-controls v3');
  }
  if (review.review_mode !== controls.review_mode) {
...
function validateReview(review) {
  ...
  if (acceptance !== null) validateAcceptance(acceptance, review.review_mode, gate);
...
function validateDecision(value) {
  ...
  const acceptance = review.acceptance === null ? null : objectValue(review.acceptance, 'acceptance');
  if (['automatic_accept', 'accept', 'accept_provisional'].includes(value.creator_action)) {
    const expectedMode = {
      automatic_accept: 'automatic',
      accept: 'manual',
      accept_provisional: 'auditable_override',
...
function validateReviewInvariants(schemaVersion, value) {
  ...
  if (schemaVersion === 'cera.pi_scene.review_checks.v1') return;
  if (schemaVersion === 'cera.pi_scene.review.v2') return validateReview(value);
  if (schemaVersion === 'cera.pi_scene.review_lifecycle.v1') return validateInitialLifecycle(value);
  if (schemaVersion === 'cera.pi_scene.review_decision.v2') return validateDecision(value);
...
def _render_python(
    ...
    typed_dicts = (
        f"{typed_dicts}\n\n\n" if typed_dicts else ""
    ) + "OrdinaryReviewDecisionV2: TypeAlias = dict[str, object]"
    functions: list[str] = []
    exported = [
        "OrdinaryReviewContractError",
        "SCHEMAS_BY_ID",
...
def normalize_{snake_name}(value: object) -> {class_name} | None:
    ...
    return f'''# Generated by scripts/generate_ordinary_review_contracts.py. DO NOT EDIT.
# ruff: noqa
# fmt: off
from __future__ import annotations
...
def _check(outputs: dict[Path, bytes]) -> int:
    ...
    if drifted:
        ...
        print("run: python scripts/generate_ordinary_review_contracts.py")
        return 1
