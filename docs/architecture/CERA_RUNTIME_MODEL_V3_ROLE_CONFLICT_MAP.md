...
| Semantic validation | independent Codex Validator | gap-free exact-span semantic ledger plus typed verdict | rewriting, repair, continuation, commit |
| Hard enforcement | deterministic Python | verified offsets/hashes/coverage, claim/cast/privacy/evidence enforcement | heuristic semantic inference |
...
| `src/cera/continuous/evidence.py` | `_story_segments` are registered as Composer declarations; Python also uses text-name heuristics as semantic fallback. | Register only Validator-owned spans, validate them mechanically against immutable Writer text, and enforce declared roles/claims/cast without regex-based semantic inference. | Correct in Stage 2. |
