...
D-224 keeps this V3 public family and adds a third backward-compatible
standing-policy pair. Historical `ordinary_provisional_continuity` v1 and v2
remain valid, while new acceptances use v3 with policy SHA-256
`6c105deb691bb3062a31b5361f09511d8e57d718cd4c2a854d81356dd2b38802`.
Each version and hash must match as one closed pair.
...
boundaries, and `cera.continuous_semantic_validator_draft.v10` for
...
candidate-specific, non-authoritative Writer recall directive. V10 adds the
closed `neutral_presentation_reference` relation only for nonpersistent
presentation narration that references Ted without asserting any action,
state, speech, response, or durable fact. The active
...
Validator adapter V19 and Reader adapter V3 submit the same anchored
...
| `cera.prepared_ingress_classifier_descriptor.v1` | Python ingress registry | Repository module, qualified implementation, exact implementation-source hash, source-unit schema, classification-receipt schema, and adapter identity for one closed prepared-ingress classifier |
| `cera.continuous_ingress_classification_receipt.v2` | Python ingress authority | Exact prepared-envelope, prepared-turn, interpretation, source-bound classifier descriptor, protected-user, raw-source, and ordered source-unit custody recomputed by the prepared-ingress bridge |
...
| `cera.continuous_ingress_receipt.v3` | Python ingress authority | Restart-safe immutable raw-source and ordered-unit custody bound to an exact verified prepared-ingress record and current classifier descriptor or a distinct frozen fixture entry plus world, branch, session, request, turn, idempotency, adapter, and protected-user identities |
...
| `cera.continuous_job4_source_frame.v1` | Python test recorder | One bounded privacy-safe repository-relative source location with function and line; no absolute path, exception text, story text, provider output, or unbounded traceback |
| `cera.continuous_job4_test_record.v1` | Python test recorder | One exact selected test ID, order, terminal status, elapsed nanoseconds, optional safe skip binding, failure/error type, hashed message, bounded source frames, and self-hash |
| `cera.continuous_job4_test_diagnostics.v1` | Python test recorder | Exact selected-test identity sequence and root, ordered per-test records and root, status counts, and closed aggregate hash; missing, duplicate, reordered, substituted, or hash-drifted records fail closed |
| `cera.continuous_job4_terminal_evidence.v6` | Python terminal custody | Additive wrapper over any historical V1-V5 terminal record plus exact selected-test diagnostics; failed/error diagnostics force terminal failure while V1-V5 remain decodeable |
...
and V8 Validator contracts enable only exact `add` and `replace` projections because
...
field-justified subject identity, approved semantic JSON path, and internal
revision. V8 rejects identity, schema, revision, owner, participant,
source/Genesis/provenance, authority, and index metadata paths and revalidates
the complete post-edit record before candidate publication. Replace also binds
the prior value hash; Python derives all operation and created-field metadata.

`cera.continuous_ingress_source_unit.v1`,
`cera.prepared_ingress_classifier_descriptor.v1`,
`cera.continuous_ingress_classification_receipt.v2`,
`cera.frozen_continuous_ingress_fixture.v1`,
`cera.continuous_ingress_receipt.v3`,
