...
// Temporary Vera debugging bridge: retain the actual latest chat entry before
// prompt assembly adds helper messages that can also use the `user` role.
let veraLatestUserMessage = null;
...
function setOpenAIMessages(chat) {
    const latestActualUser = [...chat].reverse().find(item => (
        item?.is_user === true
        && typeof item?.mes === 'string'
        && item.mes.trim().length > 0
        && !item.extra?.[IGNORE_SYMBOL]
    ));
    veraLatestUserMessage = latestActualUser?.mes?.trim() || null;
    let j = 0;
    // clean openai msgs
    const messages = [];
...
export async function createGenerationParameters(settings, model, type, messages, { jsonSchema = null } = {}) {
    ...
    const isVeraCustomModel = settings.chat_completion_source === chat_completion_sources.CUSTOM
        && ['vera-roleplay', 'vera-playable-alpha', 'vera-v5', 'vera-v6-alpha'].includes(model);
    const isCeraCustomModel = settings.chat_completion_source === chat_completion_sources.CUSTOM
        && model === 'cera-alpha';
    const isVeraPlayableAlpha = isVeraCustomModel && model === 'vera-playable-alpha';
    const isVeraV5 = isVeraCustomModel && model === 'vera-v5';
    const isVeraV6 = isVeraCustomModel && model === 'vera-v6-alpha';
    const veraAction = isVeraPlayableAlpha
        ? (type === 'swipe' ? 'swipe' : type === 'regenerate' ? 'regenerate' : type === 'continue' ? 'continue' : 'auto')
        : undefined;
    const veraGenerationId = isVeraPlayableAlpha && (type === 'swipe' || type === 'regenerate')
        ? `vera_generation_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 12)}`
        : undefined;
    const ceraRegenerationKey = isCeraCustomModel && (type === 'swipe' || type === 'regenerate')
        ? `regen_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 12)}`
        : undefined;
    ...
    const generate_data = {
        ...
        'user_name': name1,
        'char_name': name2,
        'vera_user_message': isVeraCustomModel
            ? veraLatestUserMessage
            : undefined,
        'vera_mode': isVeraCustomModel
            ? (isVeraPlayableAlpha ? 'playable_alpha' : isVeraV5 ? 'v5' : isVeraV6 ? 'v6' : 'm41')
            : undefined,
        'vera_session_id': isVeraCustomModel
            ? getVeraChatSessionId()
            : undefined,
        'vera_action': veraAction,
        'vera_generation_id': veraGenerationId,
        'cera_session_id': isCeraCustomModel
            ? getVeraChatSessionId()
            : undefined,
        'cera_scene_depth': isCeraCustomModel ? 'auto' : undefined,
...
/**
 * Return a stable Vera session key for exactly one SillyTavern chat file.
 * Chat metadata integrity is a persisted per-chat UUID; the bounded hash is a
 * fallback for the brief interval before a new chat has been saved.
 * @returns {string}
 */
function getVeraChatSessionId() {
    const integrity = typeof chat_metadata?.integrity === 'string'
        ? chat_metadata.integrity.replace(/[^a-zA-Z0-9]/g, '').toLowerCase()
        : '';
    if (integrity.length > 0) {
        return `st_chat_${integrity}`.slice(0, 64);
    }
    ...
    return `st_chat_${getStringHash(identity).toString(36)}`;
}
