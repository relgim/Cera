...
router.post('/generate', async function (request, response) {
    try {
        ...
        // Preserve opt-in local semantic provenance supplied by a client
        // extension. The downstream runtime validates the schema and hashes;
        // this bridge does not treat client metadata as trusted by itself.
        if (
            request.body.chat_completion_source === CHAT_COMPLETION_SOURCES.CUSTOM
            && request.body.runtime_semantic_provenance
            && typeof request.body.runtime_semantic_provenance === 'object'
            && !Array.isArray(request.body.runtime_semantic_provenance)
        ) {
            requestBody.runtime_semantic_provenance = request.body.runtime_semantic_provenance;
        }

        // CERA controls are transport metadata, not story-prompt instructions.
        // Forward only the validated fields for CERA's local virtual model.
        if (
            request.body.chat_completion_source === CHAT_COMPLETION_SOURCES.CUSTOM
            && request.body.model === 'cera-alpha'
        ) {
            if (
                typeof request.body.cera_session_id === 'string'
                && /^[a-z0-9][a-z0-9_-]{0,95}$/.test(request.body.cera_session_id)
            ) {
                requestBody.cera_session_id = request.body.cera_session_id;
            }
            if (
                typeof request.body.cera_scene_depth === 'string'
                && ['off', 'auto', 'long', 'epic'].includes(request.body.cera_scene_depth.toLowerCase())
            ) {
                requestBody.cera_scene_depth = request.body.cera_scene_depth.toLowerCase();
            }
            if (
                typeof request.body.cera_regeneration_key === 'string'
                && /^[a-z][a-z0-9_-]{0,95}$/.test(request.body.cera_regeneration_key)
            ) {
                requestBody.cera_regeneration_key = request.body.cera_regeneration_key;
            }
        }

        // Vera's local OpenAI-compatible bridge needs the identities that are
        // already present in this server-side request to build the correct
        // Genesis session. Keep the extra fields scoped to Vera's model so
        // unrelated Custom endpoints retain their exact request contract.
        if (
            request.body.chat_completion_source === CHAT_COMPLETION_SOURCES.CUSTOM
            && ['vera-roleplay', 'vera-playable-alpha', 'vera-v5', 'vera-v6-alpha'].includes(request.body.model)
        ) {
            requestBody.user = request.body.user_name;
            requestBody.vera_character = request.body.char_name;
            requestBody.vera_mode = request.body.model === 'vera-v6-alpha'
                ? 'v6'
                : request.body.model === 'vera-v5'
                    ? 'v5'
                    : request.body.model === 'vera-playable-alpha'
                    ? 'playable_alpha'
                    : request.body.vera_mode === 'm41'
                        ? 'm41'
                        : 'single_actor';
            if (
                typeof request.body.vera_session_id === 'string'
                && /^[a-z][a-z0-9_-]{0,63}$/.test(request.body.vera_session_id)
            ) {
                requestBody.vera_session_id = request.body.vera_session_id;
            }
            if (
                typeof request.body.vera_user_message === 'string'
                && request.body.vera_user_message.trim().length > 0
            ) {
                requestBody.vera_user_message = request.body.vera_user_message.trim();
            }
            if (
                request.body.model === 'vera-playable-alpha'
                && ['auto', 'continue', 'edit', 'regenerate', 'swipe'].includes(request.body.vera_action)
            ) {
                requestBody.vera_action = request.body.vera_action;
            }
            if (
                request.body.model === 'vera-playable-alpha'
                && typeof request.body.vera_generation_id === 'string'
                && /^[a-z][a-z0-9_-]{0,95}$/.test(request.body.vera_generation_id)
            ) {
                requestBody.vera_generation_id = request.body.vera_generation_id;
            }
        }
