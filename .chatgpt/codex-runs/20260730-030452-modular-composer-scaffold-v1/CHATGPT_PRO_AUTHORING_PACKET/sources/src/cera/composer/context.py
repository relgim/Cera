...
from cera.evidence import EvidenceFetchRequest, EvidenceSearchRequest, EvidenceService, ExactEvidence
...
CONTEXT_SELECTION_POLICY_VERSION = "cera.composer_context_selection.v2"
            ...
            for character_id in base_request.selected_npc_ids:
                ...
                if len(eligible) != 1:
                    raise ComposerContextAssemblyError(
                        ...
                        "selected character lacks one unambiguous speech-system evidence record",
                    )
                voice_reference = eligible[0]
                fetched = service.fetch_evidence(
                    snapshot,
                    EvidenceFetchRequest(
                        (voice_reference.evidence_id,),
                        ("source_text",),
                    ),
                )
                ...
                if len(fetched.exact_records) != 1:
                    raise ComposerContextAssemblyError(
                        ErrorCode.EVIDENCE_INSUFFICIENT,
                        "speech-system evidence did not expand exactly",
                    )
                voice = fetched.exact_records[0]
                ...
                continuity.append(
                    ComposerContinuityReference(
                        evidence_id=voice.evidence_id,
                        record_id=voice.metadata.record_id,
                        record_version=voice.metadata.record_version,
                    )
                )
