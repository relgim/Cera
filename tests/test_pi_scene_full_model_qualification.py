...
from cera.pi_scene.qualification import (
    ...
    QUALIFICATION_EXECUTION_POLICY_V22,
    QUALIFICATION_EXECUTION_POLICY_V31,
    ...
    QualificationFixtureV40,
    QualificationFixtureV41,
...
HISTORICAL_FIXTURES_V25 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v25.json"
)
HISTORICAL_FIXTURES_V26 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v26.json"
)
HISTORICAL_FIXTURES_V27 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v27.json"
)
HISTORICAL_FIXTURES_V28 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v28.json"
)
HISTORICAL_FIXTURES_V29 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v29.json"
)
HISTORICAL_FIXTURES_V30 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v30.json"
)
HISTORICAL_FIXTURES_V31 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v31.json"
)
HISTORICAL_FIXTURES_V32 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v32.json"
)
HISTORICAL_FIXTURES_V33 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v33.json"
)
HISTORICAL_FIXTURES_V34 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v34.json"
)
HISTORICAL_FIXTURES_V35 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v35.json"
)
HISTORICAL_FIXTURES_V36 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v36.json"
)
HISTORICAL_FIXTURES_V37 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v37.json"
)
HISTORICAL_FIXTURES_V38 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v38.json"
)
HISTORICAL_FIXTURES_V39 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v39.json"
)
HISTORICAL_FIXTURES_V40 = (
    ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v40.json"
)
FIXTURES = ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v41.json"
...
class _FakeQualificationClient:
    ...
    def __init__(
        ...
        *,
        ...
        standing_policy_after_regenerate_fixture_id: str | None = None,
        ...
        self.standing_policy_after_regenerate_fixture_id = (
            standing_policy_after_regenerate_fixture_id
        )
    ...
    def regenerate(
        ...
        *,
        ...
        standing_policy = self.standing_policy_after_regenerate_fixture_id == fixture.fixture_id
        successor_review_id = (
        ...
        self._bind_terminal_decision(
            successor_review_id,
            creator_action=(
                "standing_policy_accept_provisional" if standing_policy else "automatic_accept"
            ),
        )
...
class _PermanentTerminalDecisionNotReadyClient(_FakeQualificationClient):
    """Never expose a terminal decision, proving reconciliation stays bounded."""
...
class _TransientTerminalAcceptanceAuthorityClient(_FakeQualificationClient):
    """Return one valid terminal DTO carrying stale acceptance authority."""
    ...
    def terminal_review_decision(self, *, review_id: str) -> ClientResponseV1:
        ...
        if self.terminal_decision_reads.count(review_id) != 1:
            return response
        changed = cast(dict[str, Any], deepcopy(response.body))
        changed["creator_action"] = "repair_recording"
        changed = self._decision_with_terminal_pointer(changed, review_id=review_id)
        return ClientResponseV1(
            transport=response.transport,
            path=response.path,
...
class FullModelQualificationTests(unittest.TestCase):
    ...
    def test_manifest_v45_freezes_every_execution_policy_field(self) -> None:
        ...
        with self.assertRaisesRegex(StateConflictError, "execution policy changed"):
            validate_qualification_manifest(manifest)
        manifest = _manifest()
        manifest["execution_policy"]["ordinary_standing_creator_policy"][
            "hard_regenerate_soft_successor_tolerated"
        ] = False
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        with self.assertRaisesRegex(StateConflictError, "execution policy changed"):
    ...
    def test_historical_manifest_v22_remains_readable_without_standing_policy(self) -> None:
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V25)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V25.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V29
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V26)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V26.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V30
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V27)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V27.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V31
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V28)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V28.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V32
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V29)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V29.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V33
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V30)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V30.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V34
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V31)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V31.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V35
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V32)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V32.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V36
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V33)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V33.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V37
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V34)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V34.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V38
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V35)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V35.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V39
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V36)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V36.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V40
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V37)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V37.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V41
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V38)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V38.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V42
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V39)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V39.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V43
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V43)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)
        ...
        manifest.update(
            qualification_fixture_manifest_metadata(
                load_qualification_fixtures(HISTORICAL_FIXTURES_V40)
            )
        )
        manifest["fixture_set_sha256"] = bytes_sha256(HISTORICAL_FIXTURES_V40.read_bytes())
        manifest["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V44
        manifest["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V44)
        unsigned = {name: value for name, value in manifest.items() if name != "manifest_sha256"}
        manifest["manifest_sha256"] = canonical_sha256(unsigned)
        validate_qualification_manifest(manifest)

    def test_v41_story_capability_fixtures_are_cumulatively_novel_and_history_remains_readable(
        self,
    ) -> None:
        ...
        historical_v25 = load_qualification_fixtures(HISTORICAL_FIXTURES_V25)
        historical_v26 = load_qualification_fixtures(HISTORICAL_FIXTURES_V26)
        historical_v27 = load_qualification_fixtures(HISTORICAL_FIXTURES_V27)
        historical_v28 = load_qualification_fixtures(HISTORICAL_FIXTURES_V28)
        historical_v29 = load_qualification_fixtures(HISTORICAL_FIXTURES_V29)
        historical_v30 = load_qualification_fixtures(HISTORICAL_FIXTURES_V30)
        historical_v31 = load_qualification_fixtures(HISTORICAL_FIXTURES_V31)
        historical_v32 = load_qualification_fixtures(HISTORICAL_FIXTURES_V32)
        historical_v33 = load_qualification_fixtures(HISTORICAL_FIXTURES_V33)
        historical_v34 = load_qualification_fixtures(HISTORICAL_FIXTURES_V34)
        historical_v35 = load_qualification_fixtures(HISTORICAL_FIXTURES_V35)
        historical_v36 = load_qualification_fixtures(HISTORICAL_FIXTURES_V36)
        historical_v37 = load_qualification_fixtures(HISTORICAL_FIXTURES_V37)
        historical_v38 = load_qualification_fixtures(HISTORICAL_FIXTURES_V38)
        historical_v39 = load_qualification_fixtures(HISTORICAL_FIXTURES_V39)
        historical_v40 = load_qualification_fixtures(HISTORICAL_FIXTURES_V40)
        ...
        self.assertEqual(len(historical_v25), 30)
        self.assertEqual(len(historical_v26), 30)
        self.assertEqual(len(historical_v27), 30)
        self.assertEqual(len(historical_v28), 30)
        self.assertEqual(len(historical_v29), 30)
        self.assertEqual(len(historical_v30), 30)
        self.assertEqual(len(historical_v31), 30)
        self.assertEqual(len(historical_v32), 30)
        self.assertEqual(len(historical_v33), 30)
        self.assertEqual(len(historical_v34), 30)
        self.assertEqual(len(historical_v35), 30)
        self.assertEqual(len(historical_v36), 30)
        self.assertEqual(len(historical_v37), 30)
        self.assertEqual(len(historical_v38), 30)
        self.assertEqual(len(historical_v39), 30)
        self.assertEqual(len(historical_v40), 30)
        ...
        self.assertTrue(all(type(value) is QualificationFixtureV25 for value in historical_v25))
        self.assertTrue(all(type(value) is QualificationFixtureV26 for value in historical_v26))
        self.assertTrue(all(type(value) is QualificationFixtureV27 for value in historical_v27))
        self.assertTrue(all(type(value) is QualificationFixtureV28 for value in historical_v28))
        self.assertTrue(all(type(value) is QualificationFixtureV29 for value in historical_v29))
        self.assertTrue(all(type(value) is QualificationFixtureV30 for value in historical_v30))
        self.assertTrue(all(type(value) is QualificationFixtureV31 for value in historical_v31))
        self.assertTrue(all(type(value) is QualificationFixtureV32 for value in historical_v32))
        self.assertTrue(all(type(value) is QualificationFixtureV33 for value in historical_v33))
        self.assertTrue(all(type(value) is QualificationFixtureV34 for value in historical_v34))
        self.assertTrue(all(type(value) is QualificationFixtureV35 for value in historical_v35))
        self.assertTrue(all(type(value) is QualificationFixtureV36 for value in historical_v36))
        self.assertTrue(all(type(value) is QualificationFixtureV37 for value in historical_v37))
        self.assertTrue(all(type(value) is QualificationFixtureV38 for value in historical_v38))
        self.assertTrue(all(type(value) is QualificationFixtureV39 for value in historical_v39))
        self.assertTrue(all(type(value) is QualificationFixtureV40 for value in historical_v40))
        self.assertTrue(all(type(value) is QualificationFixtureV41 for value in stress))
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            [
                "stress:ax_be_mountain_rescue_counterfeit_beacon",
                "stress:ax_be_food_truck_recipe_prank",
                "stress:ax_be_flooded_shelter_drone_release",
                "stress:ax_be_cavern_map_false_future",
                "stress:ax_be_night_market_synthetic_stunt",
                "stress:ax_be_adult_mask_workshop_prop_roles",
                "stress:ax_be_adult_rainy_loft_mood_projector",
                "stress:ax_be_adult_vintage_letter_false_agreement",
                "stress:ax_be_adult_boathouse_interruption_pause",
                "stress:ax_be_adult_studio_false_livestream",
                "stress:ax_be_clocktower_counterfeit_restorer",
                "stress:ax_be_tidal_turbine_cloned_engineer",
                "stress:ax_be_radio_archive_synthetic_ted_line",
                "stress:ax_be_snow_day_lantern_addition",
                "stress:ax_be_ar_gallery_false_sakura_identity",
                "stress:ax_be_adult_moonlit_pool_choreography",
                "stress:ax_be_adult_snowbound_cabin_heat_sensor",
                "stress:ax_be_adult_planetarium_archival_moment",
                "stress:ax_be_adult_orchard_power_pause",
                "stress:ax_be_adult_gallery_false_booking",
                "stress:ax_st_clockwork_museum_injection",
                "stress:ax_st_drone_race_synthetic_ted",
                "stress:ax_st_orchard_trip_music_addition",
                "stress:ax_st_adult_costume_loft_prompt_card",
                "stress:ax_st_adult_houseboat_color_sensor",
                "stress:ax_st_adult_darkroom_chemical_alarm",
                "stress:ax_st_false_sakura_flight_identity",
                "stress:ax_st_false_hana_mia_recipe_archive",
                "stress:ax_st_adult_jazz_club_audience_pressure",
                "stress:ax_st_adult_train_observation_pause",
            ],
        )
        self.assertEqual(
            [value.stress_tags for value in stress],
            [value.stress_tags for value in historical_v40],
        )
        self.assertEqual(
            [
                (
        ...
        self.assertFalse({value.user_source for value in historical_v25} & current_sources)
        self.assertFalse({value.user_source for value in historical_v26} & current_sources)
        self.assertFalse({value.user_source for value in historical_v27} & current_sources)
        self.assertFalse({value.user_source for value in historical_v28} & current_sources)
        self.assertFalse({value.user_source for value in historical_v29} & current_sources)
        self.assertFalse({value.user_source for value in historical_v30} & current_sources)
        self.assertFalse({value.user_source for value in historical_v31} & current_sources)
        self.assertFalse({value.user_source for value in historical_v32} & current_sources)
        self.assertFalse({value.user_source for value in historical_v33} & current_sources)
        self.assertFalse({value.user_source for value in historical_v34} & current_sources)
        self.assertFalse({value.user_source for value in historical_v35} & current_sources)
        self.assertFalse({value.user_source for value in historical_v36} & current_sources)
        self.assertFalse({value.user_source for value in historical_v37} & current_sources)
        self.assertFalse({value.user_source for value in historical_v38} & current_sources)
        self.assertFalse({value.user_source for value in historical_v39} & current_sources)
        self.assertFalse({value.user_source for value in historical_v40} & current_sources)
        self.assertFalse(
        ...
        self.assertFalse(
            {value.novelty_id for value in historical_v25} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v26} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v27} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v28} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v29} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v30} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v31} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v32} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v33} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v34} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v35} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v36} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v37} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v38} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v39} & {value.novelty_id for value in stress}
        )
        self.assertFalse(
            {value.novelty_id for value in historical_v40} & {value.novelty_id for value in stress}
        )
        self.assertEqual(
        ...
        self.assertTrue(
            all("may take plausible physical actions" in value for value in adult_sources)
        )
        self.assertTrue(all("speak limited dialogue" in value for value in adult_sources))
        self.assertTrue(
            all("do not invent his private thoughts" in value for value in adult_sources)
        )
        ...
        self.assertEqual(len(active_adult_sources), 15)
        self.assertTrue(all("capacity" in value for value in active_adult_sources))
        self.assertTrue(all("may stop freely" in value for value in active_adult_sources))
        self.assertTrue(
            all("withdrawal takes effect immediately" in value for value in active_adult_sources)
        )
        ordinary_sources = [
            value.user_source.lower()
            for value in stress
        ...
        self.assertTrue(
            all(
                any(marker in value for marker in provisional_markers) for value in ordinary_sources
            )
        )
        ...
        self.assertEqual(len(ancestral_source_hashes), 1200)
        ...
        self.assertEqual(
            ...
            "6732a732123a5f3ddb7fdcc028517cb8783bd448715493b5e4bbb3d3686a6b1a",
        )
        self.assertEqual(
            bytes_sha256(FIXTURES.read_bytes()),
            "bc37c1cf8d9755a4d5a2d39669bf929dba8ff71807dd61120b58c0d94e16287c",
        )
        self.assertEqual(
            metadata["fixture_baseline"]["path"],
            HISTORICAL_FIXTURES_V40.name,
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            [
                ...
                HISTORICAL_FIXTURES_V25.name,
                HISTORICAL_FIXTURES_V26.name,
                HISTORICAL_FIXTURES_V27.name,
                HISTORICAL_FIXTURES_V28.name,
                HISTORICAL_FIXTURES_V29.name,
                HISTORICAL_FIXTURES_V30.name,
                HISTORICAL_FIXTURES_V31.name,
                HISTORICAL_FIXTURES_V32.name,
                HISTORICAL_FIXTURES_V33.name,
                HISTORICAL_FIXTURES_V34.name,
                HISTORICAL_FIXTURES_V35.name,
                HISTORICAL_FIXTURES_V36.name,
                HISTORICAL_FIXTURES_V37.name,
                HISTORICAL_FIXTURES_V38.name,
                HISTORICAL_FIXTURES_V39.name,
                HISTORICAL_FIXTURES_V40.name,
            ],
        )
    ...
    def test_v41_story_capability_fixture_contract_and_cumulative_ancestry_fail_closed(
        self,
    ) -> None:
        source = json.loads(FIXTURES.read_text(encoding="utf-8"))
        historical_v40 = json.loads(HISTORICAL_FIXTURES_V40.read_text(encoding="utf-8"))
        cases: dict[str, Callable[[dict[str, Any]], None]] = {
            "baseline hash": lambda value: value.__setitem__("baseline_fixture_sha256", "0" * 64),
            "ancestry hash": lambda value: value["baseline_ancestry"][39].__setitem__(
                "sha256", "0" * 64
            ),
            "ancestry order": lambda value: value.__setitem__(
                "baseline_ancestry", list(reversed(value["baseline_ancestry"]))
            ...
            "v40 source reuse": lambda value: value["fixtures"][0].__setitem__(
                "user_source", historical_v40["fixtures"][0]["user_source"]
            ),
            "v40 novelty reuse": lambda value: value["fixtures"][0].__setitem__(
                "novelty_id", historical_v40["fixtures"][0]["novelty_id"]
            ),
        }
        for label, mutate in cases.items():
            with self.subTest(label=label), tempfile.TemporaryDirectory() as temporary:
    ...
    def test_v41_rejects_a_self_consistent_noncanonical_v40_baseline(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            for ancestor in (
                LEGACY_FIXTURES,
                ...
                shutil.copy2(ancestor, root / ancestor.name)
            historical = json.loads(HISTORICAL_FIXTURES_V40.read_text(encoding="utf-8"))
            historical["fixtures"][0]["user_source"] += " Altered V40 marker."
            historical_path = root / HISTORICAL_FIXTURES_V40.name
            historical_path.write_text(json.dumps(historical), encoding="utf-8")
            changed_hash = bytes_sha256(historical_path.read_bytes())
            current = json.loads(FIXTURES.read_text(encoding="utf-8"))
            current["baseline_fixture_sha256"] = changed_hash
            current["baseline_ancestry"][39]["sha256"] = changed_hash
    ...
    def test_spent_v41_fixture_manifest_rejects_renamed_sources_and_preserves_closure(self) -> None:
        prior = build_qualification_manifest(
            repository_root=ROOT,
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            shutil.copy2(HISTORICAL_FIXTURES_V25, root / HISTORICAL_FIXTURES_V25.name)
            shutil.copy2(HISTORICAL_FIXTURES_V26, root / HISTORICAL_FIXTURES_V26.name)
            shutil.copy2(HISTORICAL_FIXTURES_V27, root / HISTORICAL_FIXTURES_V27.name)
            shutil.copy2(HISTORICAL_FIXTURES_V28, root / HISTORICAL_FIXTURES_V28.name)
            shutil.copy2(HISTORICAL_FIXTURES_V29, root / HISTORICAL_FIXTURES_V29.name)
            shutil.copy2(HISTORICAL_FIXTURES_V30, root / HISTORICAL_FIXTURES_V30.name)
            shutil.copy2(HISTORICAL_FIXTURES_V31, root / HISTORICAL_FIXTURES_V31.name)
            shutil.copy2(HISTORICAL_FIXTURES_V32, root / HISTORICAL_FIXTURES_V32.name)
            shutil.copy2(HISTORICAL_FIXTURES_V33, root / HISTORICAL_FIXTURES_V33.name)
            shutil.copy2(HISTORICAL_FIXTURES_V34, root / HISTORICAL_FIXTURES_V34.name)
            shutil.copy2(HISTORICAL_FIXTURES_V35, root / HISTORICAL_FIXTURES_V35.name)
            shutil.copy2(HISTORICAL_FIXTURES_V36, root / HISTORICAL_FIXTURES_V36.name)
            shutil.copy2(HISTORICAL_FIXTURES_V37, root / HISTORICAL_FIXTURES_V37.name)
            shutil.copy2(HISTORICAL_FIXTURES_V38, root / HISTORICAL_FIXTURES_V38.name)
            shutil.copy2(HISTORICAL_FIXTURES_V39, root / HISTORICAL_FIXTURES_V39.name)
            shutil.copy2(HISTORICAL_FIXTURES_V40, root / HISTORICAL_FIXTURES_V40.name)
    ...
    def test_v45_closure_ingests_root_g_v5_through_at_v44(self) -> None:
        ...
        manifest_v5["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v6["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v7["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v8["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v9["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v10["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v11["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v12["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v13["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v14["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v15["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v16["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v17["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v18["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v19["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v20["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v21["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v22["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V22)
        ...
        manifest_v24["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V31)
        ...
        manifest_v25["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V31)
        ...
        manifest_v26["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V31)
        ...
        manifest_v27["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V31)
        ...
        manifest_v28["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V31)
        ...
        manifest_v29 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-ad-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V25,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V25,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V25,)},
            spent_manifests=(manifest_v28,),
        )
        manifest_v29["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V29
        manifest_v29["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V31)
        manifest_v29["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v29.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v29)
        manifest_v30 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-ae-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V26,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V26,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V26,)},
            spent_manifests=(manifest_v29,),
        )
        manifest_v30["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V30
        manifest_v30["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V31)
        manifest_v30["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v30.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v30)
        manifest_v31 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-af-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V27,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V27,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V27,)},
            spent_manifests=(manifest_v30,),
        )
        manifest_v31["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V31
        manifest_v31["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V31)
        manifest_v31["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v31.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v31)
        manifest_v32 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-ag-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V28,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V28,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V28,)},
            spent_manifests=(manifest_v31,),
        )
        manifest_v32["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V32
        manifest_v32["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        manifest_v32["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v32.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v32)
        manifest_v33 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-ah-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V29,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V29,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V29,)},
            spent_manifests=(manifest_v32,),
        )
        manifest_v33["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V33
        manifest_v33["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        manifest_v33["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v33.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v33)
        manifest_v34 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-ai-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V30,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V30,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V30,)},
            spent_manifests=(manifest_v33,),
        )
        manifest_v34["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V34
        manifest_v34["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        manifest_v34["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v34.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v34)
        manifest_v35 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-aj-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V31,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V31,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V31,)},
            spent_manifests=(manifest_v34,),
        )
        manifest_v35["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V35
        manifest_v35["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        manifest_v35["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v35.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v35)
        manifest_v36 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-al-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V32,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V32,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V32,)},
            spent_manifests=(manifest_v35,),
        )
        manifest_v36["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V36
        manifest_v36["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        manifest_v36["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v36.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v36)
        manifest_v37 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-am-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V33,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V33,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V33,)},
            spent_manifests=(manifest_v36,),
        )
        manifest_v37["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V37
        manifest_v37["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        manifest_v37["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v37.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v37)
        manifest_v38 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-an-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V34,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V34,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V34,)},
            spent_manifests=(manifest_v37,),
        )
        manifest_v38["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V38
        manifest_v38["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        manifest_v38["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v38.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v38)
        manifest_v39 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-ao-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V35,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V35,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V35,)},
            spent_manifests=(manifest_v38,),
        )
        manifest_v39["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V39
        manifest_v39["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        manifest_v39["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v39.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v39)
        manifest_v40 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-ap-source-20260814",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V36,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V36,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V36,)},
            spent_manifests=(manifest_v39,),
        )
        manifest_v40["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V40
        manifest_v40["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        manifest_v40["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v40.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v40)
        manifest_v41 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-aq-source-20260815",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V37,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V37,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V37,)},
            spent_manifests=(manifest_v40,),
        )
        manifest_v41["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V41
        manifest_v41["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        manifest_v41["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v41.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v41)
        manifest_v42 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-ar-source-20260815",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V38,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V38,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V38,)},
            spent_manifests=(manifest_v41,),
        )
        manifest_v42["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V42
        manifest_v42["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V42)
        manifest_v42["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v42.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v42)
        manifest_v43 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-as-source-20260815",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V39,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V39,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V39,)},
            spent_manifests=(manifest_v42,),
        )
        manifest_v43["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V43
        manifest_v43["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V43)
        manifest_v43["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v43.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v43)
        manifest_v44 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-at-source-20260815",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=HISTORICAL_FIXTURES_V40,
            repository_artifacts={"fixture": (HISTORICAL_FIXTURES_V40,)},
            external_artifacts={"external_fixture": (HISTORICAL_FIXTURES_V40,)},
            spent_manifests=(manifest_v43,),
        )
        manifest_v44["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V44
        manifest_v44["execution_policy"] = deepcopy(QUALIFICATION_EXECUTION_POLICY_V44)
        manifest_v44["manifest_sha256"] = canonical_sha256(
            {key: value for key, value in manifest_v44.items() if key != "manifest_sha256"}
        )
        validate_qualification_manifest(manifest_v44)
        manifest_v45 = build_qualification_manifest(
            repository_root=ROOT,
            qualification_id="qualification-root-au-source-20260815",
            source_commit="a" * 40,
            source_tree="b" * 40,
            fixture_path=FIXTURES,
            repository_artifacts={"fixture": (FIXTURES,)},
            external_artifacts={"external_fixture": (FIXTURES,)},
            spent_manifests=(manifest_v44,),
        )
        self.assertEqual(manifest_v45["schema_version"], QUALIFICATION_MANIFEST_SCHEMA)
        self.assertEqual(
            set(manifest_v45["spent_manifest_sha256s"]),
            {
                ...
                manifest_v29["manifest_sha256"],
                manifest_v30["manifest_sha256"],
                manifest_v31["manifest_sha256"],
                manifest_v32["manifest_sha256"],
                manifest_v33["manifest_sha256"],
                manifest_v34["manifest_sha256"],
                manifest_v35["manifest_sha256"],
                manifest_v36["manifest_sha256"],
                manifest_v37["manifest_sha256"],
                manifest_v38["manifest_sha256"],
                manifest_v39["manifest_sha256"],
                manifest_v40["manifest_sha256"],
                manifest_v41["manifest_sha256"],
                manifest_v42["manifest_sha256"],
                manifest_v43["manifest_sha256"],
                manifest_v44["manifest_sha256"],
            },
        )
        self.assertEqual(
            set(manifest_v45["spent_fixture_set_sha256s"]),
            {
                ...
                manifest_v29["fixture_set_sha256"],
                manifest_v30["fixture_set_sha256"],
                manifest_v31["fixture_set_sha256"],
                manifest_v32["fixture_set_sha256"],
                manifest_v33["fixture_set_sha256"],
                manifest_v34["fixture_set_sha256"],
                manifest_v35["fixture_set_sha256"],
                manifest_v36["fixture_set_sha256"],
                manifest_v37["fixture_set_sha256"],
                manifest_v38["fixture_set_sha256"],
                manifest_v39["fixture_set_sha256"],
                manifest_v40["fixture_set_sha256"],
                manifest_v41["fixture_set_sha256"],
                manifest_v42["fixture_set_sha256"],
                manifest_v43["fixture_set_sha256"],
                manifest_v44["fixture_set_sha256"],
            },
        )
        self.assertEqual(len(manifest_v45["spent_manifest_sha256s"]), 39)
        self.assertEqual(len(manifest_v45["spent_fixture_set_sha256s"]), 39)
        expected_novelty = {
            value["novelty_id"]
            for manifest in (
                manifest_v5,
        ...
        self.assertEqual(set(manifest_v45["spent_novelty_ids"]), expected_novelty)
        self.assertEqual(set(manifest_v45["spent_source_sha256s"]), expected_sources)
        self.assertEqual(len(manifest_v45["spent_novelty_ids"]), 1170)
        self.assertEqual(len(manifest_v45["spent_source_sha256s"]), 1170)
        with tempfile.TemporaryDirectory() as temporary:
            ...
            sibling_v29 = parent / "root-zz-ad-v29" / "QUALIFICATION_MANIFEST.json"
            sibling_v29.parent.mkdir()
            sibling_v29.write_bytes(canonical_bytes(manifest_v29) + b"\n")
            sibling_v30 = parent / "root-zz-ae-v30" / "QUALIFICATION_MANIFEST.json"
            sibling_v30.parent.mkdir()
            sibling_v30.write_bytes(canonical_bytes(manifest_v30) + b"\n")
            sibling_v31 = parent / "root-zz-af-v31" / "QUALIFICATION_MANIFEST.json"
            sibling_v31.parent.mkdir()
            sibling_v31.write_bytes(canonical_bytes(manifest_v31) + b"\n")
            sibling_v32 = parent / "root-zz-ag-v32" / "QUALIFICATION_MANIFEST.json"
            sibling_v32.parent.mkdir()
            sibling_v32.write_bytes(canonical_bytes(manifest_v32) + b"\n")
            sibling_v33 = parent / "root-zz-ah-v33" / "QUALIFICATION_MANIFEST.json"
            sibling_v33.parent.mkdir()
            sibling_v33.write_bytes(canonical_bytes(manifest_v33) + b"\n")
            sibling_v34 = parent / "root-zz-ai-v34" / "QUALIFICATION_MANIFEST.json"
            sibling_v34.parent.mkdir()
            sibling_v34.write_bytes(canonical_bytes(manifest_v34) + b"\n")
            sibling_v35 = parent / "root-zz-aj-v35" / "QUALIFICATION_MANIFEST.json"
            sibling_v35.parent.mkdir()
            sibling_v35.write_bytes(canonical_bytes(manifest_v35) + b"\n")
            sibling_v36 = parent / "root-zz-al-v36" / "QUALIFICATION_MANIFEST.json"
            sibling_v36.parent.mkdir()
            sibling_v36.write_bytes(canonical_bytes(manifest_v36) + b"\n")
            sibling_v37 = parent / "root-zz-am-v37" / "QUALIFICATION_MANIFEST.json"
            sibling_v37.parent.mkdir()
            sibling_v37.write_bytes(canonical_bytes(manifest_v37) + b"\n")
            sibling_v38 = parent / "root-zz-an-v38" / "QUALIFICATION_MANIFEST.json"
            sibling_v38.parent.mkdir()
            sibling_v38.write_bytes(canonical_bytes(manifest_v38) + b"\n")
            sibling_v39 = parent / "root-zz-ao-v39" / "QUALIFICATION_MANIFEST.json"
            sibling_v39.parent.mkdir()
            sibling_v39.write_bytes(canonical_bytes(manifest_v39) + b"\n")
            sibling_v40 = parent / "root-zz-ap-v40" / "QUALIFICATION_MANIFEST.json"
            sibling_v40.parent.mkdir()
            sibling_v40.write_bytes(canonical_bytes(manifest_v40) + b"\n")
            sibling_v41 = parent / "root-zz-aq-v41" / "QUALIFICATION_MANIFEST.json"
            sibling_v41.parent.mkdir()
            sibling_v41.write_bytes(canonical_bytes(manifest_v41) + b"\n")
            sibling_v42 = parent / "root-zz-ar-v42" / "QUALIFICATION_MANIFEST.json"
            sibling_v42.parent.mkdir()
            sibling_v42.write_bytes(canonical_bytes(manifest_v42) + b"\n")
            sibling_v43 = parent / "root-zz-as-v43" / "QUALIFICATION_MANIFEST.json"
            sibling_v43.parent.mkdir()
            sibling_v43.write_bytes(canonical_bytes(manifest_v43) + b"\n")
            sibling_v44 = parent / "root-zz-at-v44" / "QUALIFICATION_MANIFEST.json"
            sibling_v44.parent.mkdir()
            sibling_v44.write_bytes(canonical_bytes(manifest_v44) + b"\n")
    ...
    def test_v45_manifest_rejects_v41_schema_or_baseline_tampering(self) -> None:
        manifest = _manifest()
        self.assertEqual(manifest["schema_version"], QUALIFICATION_MANIFEST_SCHEMA)

        downgraded = deepcopy(manifest)
        downgraded["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V44
        ...
        rebound["fixture_baseline"] = {
            "path": HISTORICAL_FIXTURES_V39.name,
            "sha256": bytes_sha256(HISTORICAL_FIXTURES_V39.read_bytes()),
        }
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for fixture, result in zip(
                ...
                self.assertEqual(
                    ...
                    "cera.pi_scene.full_model_qualification_fixtures.v41",
                )
    ...
    def test_hard_regenerate_accepts_one_soft_provisional_successor(self) -> None:
        fixtures = load_qualification_fixtures(FIXTURES)
        fixture_id = "backend-ordinary-01"
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ...
            client = _FakeQualificationClient(
                runtime,
                reject_first=True,
                standing_policy_after_regenerate_fixture_id=fixture_id,
            )
            campaign = runner.start_phase(QualificationPhase.BACKEND, fixtures)
            campaign.run_segment(client=client, runtime_root=runtime, turn_count=1)
            ...
            accepted = next(
                value for value in client.reviews.values() if value["state"] == "accepted"
            )
            self.assertEqual(accepted["acceptance"]["mode"], "standing_policy")
            self.assertEqual(accepted["acceptance"]["canon_status"], "provisional")
            self.assertEqual(len(accepted["provider_attempts"]), 2)
            self.assertEqual(
                [value["disposition"] for value in accepted["provider_attempts"]],
                ["luna_rejected", "luna_rejected"],
            )
            ...
            self.assertEqual(
                sum(
                    value.get("event") == "regenerate_successor_policy_provisional"
    ...
    def test_manifest_verification_fails_closed_after_artifact_drift(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            ...
            historical_v25 = root / HISTORICAL_FIXTURES_V25.name
            historical_v25.write_bytes(HISTORICAL_FIXTURES_V25.read_bytes())
            historical_v26 = root / HISTORICAL_FIXTURES_V26.name
            historical_v26.write_bytes(HISTORICAL_FIXTURES_V26.read_bytes())
            historical_v27 = root / HISTORICAL_FIXTURES_V27.name
            historical_v27.write_bytes(HISTORICAL_FIXTURES_V27.read_bytes())
            historical_v28 = root / HISTORICAL_FIXTURES_V28.name
            historical_v28.write_bytes(HISTORICAL_FIXTURES_V28.read_bytes())
            historical_v29 = root / HISTORICAL_FIXTURES_V29.name
            ...
            manifest = build_qualification_manifest(
                ...
                repository_artifacts={
                    "test": (
                        ...
                        Path(HISTORICAL_FIXTURES_V25.name),
                        Path(HISTORICAL_FIXTURES_V26.name),
                        Path(HISTORICAL_FIXTURES_V27.name),
    ...
    def test_terminal_review_snapshot_mismatch_reconciles_with_get_only(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            runtime = root / "runtime"
            client = _PermanentTerminalDecisionNotReadyClient(runtime)
            campaign = FullModelQualificationRunner(
                manifest=_manifest(),
                runtime_root=runtime,
                evidence_root=root / "evidence",
            ...
            with (
                ...
                patch(
                    "cera.pi_scene.qualification.PROVIDER_STAGE_RETRY_STATUS_TIMEOUT_SECONDS",
                    0,
                ),
            ):
                campaign.run_segment(
            ...
            self.assertEqual(
                str(campaign.failure),
                "ordinary terminal decision GET returned HTTP 404",
            )
            self.assertEqual(client.regenerates, 0)
            self.assertEqual(client.review_actions, [])
