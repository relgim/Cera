...
FIXTURES = ROOT / "evaluation" / "fixtures" / "pi_scene_full_model_qualification_v31.json"
...
class FullModelQualificationTests(unittest.TestCase):
    ...
    def test_manifest_v35_freezes_every_execution_policy_field(self) -> None:
        mutations = {
    ...
    def test_v31_practical_fixtures_are_cumulatively_novel_and_history_remains_readable(
        self,
    ) -> None:
        ...
        self.assertTrue(all(type(value) is QualificationFixtureV31 for value in stress))
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            [
                "stress:v31_be_planetarium_crosscampus_badge",
                "stress:v31_be_floodcontrol_predelivery_work_order",
                "stress:v31_be_music_archive_absent_conservator",
                "stress:v31_be_geologic_core_mixed_custody",
                "stress:v31_be_volumetric_mineral_false_choice",
                "stress:v31_be_adult_botanical_adjacent_card",
                "stress:v31_be_adult_temperature_sensor_not_consent",
                "stress:v31_be_adult_gallery_reservation_not_consent",
                "stress:v31_be_adult_refrigerant_alarm_withdrawal",
                "stress:v31_be_adult_botanical_recording_close",
                "stress:v31_be_aviation_crossauthority_credential",
                "stress:v31_be_ultraviolet_spectrometer_claim",
                "stress:v31_be_textile_archive_absent_registrar",
                "stress:v31_be_future_aurora_recording",
                "stress:v31_be_generated_audio_rarebook_release",
                "stress:v31_be_adult_printmaking_adjacent_script",
                "stress:v31_be_adult_pulse_sensor_not_consent",
                "stress:v31_be_adult_printmaking_permit_window",
                "stress:v31_be_adult_solventfire_alarm_withdrawal",
                "stress:v31_be_adult_printmaking_recording_close",
                "stress:v31_st_planetarium_bulletin_injection",
                "stress:v31_st_projection_archive_false_choice",
                "stress:v31_st_magnetictape_archive_false_history",
                "stress:v31_st_adult_rehearsal_prop_sheet",
                "stress:v31_st_adult_breathing_sensor_not_consent",
                "stress:v31_st_adult_gas_alarm_withdrawal",
                "stress:v31_st_library_identity_age_forgery",
                "stress:v31_st_hospital_maternal_relation_forgery",
                "stress:v31_st_adult_gallery_deadline_pressure",
                "stress:v31_st_adult_power_alarm_withdrawal",
            ],
        )
        self.assertEqual(
            [value.stress_tags for value in stress],
            [value.stress_tags for value in historical_v30],
        )
        self.assertEqual(
            [
                (
        ...
        self.assertEqual(
            ...
            "6dcd6a53b9e45817882d27e6dd44ed81aa86ad346ff8db855f5aeb03c5cc63bb",
        )
        self.assertEqual(
    ...
    def test_v31_practical_fixture_contract_and_cumulative_ancestry_fail_closed(self) -> None:
        source = json.loads(FIXTURES.read_text(encoding="utf-8"))
        historical_v30 = json.loads(HISTORICAL_FIXTURES_V30.read_text(encoding="utf-8"))
        cases: dict[str, Callable[[dict[str, Any]], None]] = {
            "baseline hash": lambda value: value.__setitem__("baseline_fixture_sha256", "0" * 64),
            "ancestry hash": lambda value: value["baseline_ancestry"][29].__setitem__(
                "sha256", "0" * 64
            ),
            "ancestry order": lambda value: value.__setitem__(
                "baseline_ancestry", list(reversed(value["baseline_ancestry"]))
            ...
            "v30 source reuse": lambda value: value["fixtures"][0].__setitem__(
                "user_source", historical_v30["fixtures"][0]["user_source"]
            ),
            "v30 novelty reuse": lambda value: value["fixtures"][0].__setitem__(
                "novelty_id", historical_v30["fixtures"][0]["novelty_id"]
            ),
        }
        for label, mutate in cases.items():
            with self.subTest(label=label), tempfile.TemporaryDirectory() as temporary:
    ...
    def test_v31_rejects_a_self_consistent_noncanonical_v30_baseline(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            for ancestor in (
                LEGACY_FIXTURES,
                ...
                shutil.copy2(ancestor, root / ancestor.name)
            historical = json.loads(HISTORICAL_FIXTURES_V30.read_text(encoding="utf-8"))
            historical["fixtures"][0]["user_source"] += " Altered V30 marker."
            historical_path = root / HISTORICAL_FIXTURES_V30.name
            historical_path.write_text(json.dumps(historical), encoding="utf-8")
            changed_hash = bytes_sha256(historical_path.read_bytes())
            current = json.loads(FIXTURES.read_text(encoding="utf-8"))
            current["baseline_fixture_sha256"] = changed_hash
            current["baseline_ancestry"][29]["sha256"] = changed_hash
            current_path = root / FIXTURES.name
            current_path.write_text(json.dumps(current), encoding="utf-8")
            with self.assertRaisesRegex(ContractValidationError, "baseline hash"):
                load_qualification_fixtures(current_path)

    def test_spent_v31_fixture_manifest_rejects_fixture_or_novelty_reuse(self) -> None:
    ...
    def test_spent_v31_fixture_manifest_rejects_renamed_sources_and_preserves_closure(self) -> None:
        prior = build_qualification_manifest(
            repository_root=ROOT,
    ...
    def test_v35_closure_ingests_root_g_v5_through_ai_v34(self) -> None:
        ...
        self.assertEqual(manifest_v35["schema_version"], QUALIFICATION_MANIFEST_SCHEMA)
        self.assertEqual(
            set(manifest_v35["spent_manifest_sha256s"]),
            {
        ...
        self.assertEqual(
            set(manifest_v35["spent_fixture_set_sha256s"]),
            {
        ...
        self.assertEqual(len(manifest_v35["spent_manifest_sha256s"]), 29)
        self.assertEqual(len(manifest_v35["spent_fixture_set_sha256s"]), 29)
        expected_novelty = {
            value["novelty_id"]
            for manifest in (
                manifest_v5,
        ...
        self.assertEqual(set(manifest_v35["spent_novelty_ids"]), expected_novelty)
        self.assertEqual(set(manifest_v35["spent_source_sha256s"]), expected_sources)
        self.assertEqual(len(manifest_v35["spent_novelty_ids"]), 870)
        self.assertEqual(len(manifest_v35["spent_source_sha256s"]), 870)
        with tempfile.TemporaryDirectory() as temporary:
    ...
    def test_v35_manifest_rejects_v31_schema_or_baseline_tampering(self) -> None:
        manifest = _manifest()
        self.assertEqual(manifest["schema_version"], QUALIFICATION_MANIFEST_SCHEMA)

        downgraded = deepcopy(manifest)
        downgraded["schema_version"] = QUALIFICATION_MANIFEST_SCHEMA_V34
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for fixture, result in zip(
                ...
                self.assertEqual(
                    ...
                    "cera.pi_scene.full_model_qualification_fixtures.v31",
                )
