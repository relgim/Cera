...
_CREATOR_REVISION_SYSTEM_INSTRUCTION = (
    "creator_revision is out-of-world creator control, never story prose, Ted "
    "dialogue, character knowledge, or canon. The rejected_candidate_text is "
    "noncanonical evidence of what to replace, not continuity. For "
    "composer_rewrite, preserve the exact validated decision and produce one "
    "complete replacement reply that directly addresses the creator feedback "
    "and safe review codes. For a revised Reasoner plan, realize the new complete "
    "plan and do not restore rejected prior logic. Never mention the review, "
    "feedback, candidate, or revision workflow in story prose."
)
...
class DeepSeekSceneComposerPort(SceneComposerPort):
    ...
    def __init__(
        ...
        *,
        ...
        if type(thinking_enabled) is not bool:
            raise ContractValidationError(
                "DeepSeek Composer thinking flag must be boolean"
            )
    ...
    def compose(self, request: SceneComposerRequest) -> SceneComposerAdapterCall:
        ...
        packet = build_deepseek_composer_packet(
            ...
            target_provider_model=self.transport.route.model_name,
        )
        ...
        adapter_evidence = {
            ...
            "route_sha256": self.transport.route.route_sha256,
            "thinking_enabled": self.thinking_enabled,
        }
        return SceneComposerAdapterCall(
            ...
            adapter_version=DEEPSEEK_COMPOSER_ADAPTER_VERSION,
            adapter_evidence_id=(
                f"{self.transport.route.route_id}:"
                f"thinking_{'enabled' if self.thinking_enabled else 'disabled'}"
            ),
            adapter_evidence_sha256=domain_sha256(
                "cera.deepseek_scene_composer.adapter_evidence.v1",
                adapter_evidence,
            ),
...
def build_deepseek_composer_packet(
    ...
    *,
    ...
    plan = request.reasoner_outcome.behavioral_scene_plan
    if plan is not None:
        selection = _runtime_prompt_selection(request, target_provider_model)
        compiled_prompt = compile_runtime_system_prompt(selection)
        packet["prompt_selection"] = to_primitive(selection)
        packet["prompt_compilation_receipt"] = runtime_prompt_receipt_payload(
            compiled_prompt.receipt
        )
    return packet
...
def _minimal_composition_dto(
    ...
    payload = {
        ...
        "protected_user_visibility_contract": (
            _protected_user_visibility_contract(request)
        ),
        ...
        "realization_context": {
            ...
            "effective_character_projections": [
                {
                    "character_id": str(value.character_id),
                    "projection_sha256": value.projection_sha256,
                    "invariant_record_ids": [
                        str(item) for item in value.invariant_record_ids
                    ],
                    "current_state_record_ids": [
                        str(item) for item in value.current_state_record_ids
                    ],
                    "voice_record_ids": [
                        str(item) for item in value.voice_record_ids
                    ],
                    "development_record_ids": [
                        str(item) for item in value.development_record_ids
                    ],
                    "selected_section_names": list(value.selected_section_names),
                    "explicit_unknowns": list(value.explicit_unknowns),
                    "prohibited_inferences": list(value.prohibited_inferences),
                }
                for value in context.effective_character_projections
            ],
            "explicit_exclusions": list(context.explicit_exclusions),
        },
    }
    if request.reasoner_outcome.behavioral_scene_plan is not None:
        payload["behavioral_scene_plan"] = _minimal_behavioral_scene_plan(
            request.reasoner_outcome.behavioral_scene_plan
        )
    if request.creator_revision is not None:
        payload["creator_revision"] = to_primitive(request.creator_revision)
    return payload


def _protected_user_visibility_contract(
    request: SceneComposerRequest,
) -> dict[str, object]:
    """Compile a closed-world Ted-description whitelist for the Composer.

    The same source and plan data remain authoritative. This projection makes
    their protected-user consequence explicit for weaker prose models without
    transferring any semantic ownership to the provider.
    """
    ...
    source_units = (
        source.protected_envelope.units
        if source.protected_envelope is not None
        else source.ordinary_units
    )
    protected_kinds = {
    ...
    authorized_claims = [
        {
            ...
            "source_unit_id": str(claim.source_unit_id),
            "kind": claim.kind.value,
            "normalized_meaning": claim.normalized_meaning,
        }
        for claim in (() if plan is None else plan.claim_ledger.claims)
        if claim.authority is SourceClaimAuthority.SOURCE_AUTHORIZED
        and (
            claim.kind in protected_kinds
            or protected_user_id in claim.affected_character_ids
        )
    ]
    return {
        "policy": "closed_world_exact_source_or_allowance_only",
        "rendering_mode": (
            "exact_source_realization"
            if request.creator_event_coverage_required
            else "npc_reaction_reference_only"
        ),
        "authorized_source_units": [
            {
                "source_unit_id": str(unit.source_unit_id),
                "exact_text": unit.exact_text,
                "allowed_kinds": [
                    value.value for value in unit.protected_user_allowed_kinds
                ],
            }
            for unit in source_units
        ],
        "authorized_claims": authorized_claims,
        "authorized_block_allowances": [
            {
                "block_id": str(block.block_id),
                "allowed_kinds": [
                    value.value
                    for value in block.protected_user_allowance.allowed_kinds
                ],
                "source_claim_ids": [
                    str(value)
                    for value in block.protected_user_allowance.source_claim_ids
                ],
                "explanation": block.protected_user_allowance.explanation,
            }
            for block in (() if plan is None else plan.event_blocks)
        ],
        "unsupported_detail_policy": "omit",
        "npc_perception_grants_new_ted_fact": False,
        "applies_to": [
            "narration",
            "npc_dialogue_assertions",
            "npc_interiority_assertions",
            "staging",
            "sensory_description",
            "metaphor_and_simile",
        ],
...
def _runtime_prompt_selection(
    request: SceneComposerRequest,
    target_provider_model: str,
) -> PromptSelectionRequest:
    plan = request.reasoner_outcome.behavioral_scene_plan
    if plan is None:
        raise ContractValidationError("runtime modular prompt requires behavioral scene plan")
    need = request.reasoner_outcome.adult_craft_need
    adult_mode = (
        AdultRenderingMode.OFF
        if need is None
        else AdultRenderingMode(need.mode.value)
    )
    return PromptSelectionRequest(
        schema_version=PromptSelectionRequest.SCHEMA_VERSION,
        scene_depth_mode=request.scene_depth_mode,
        adult_rendering_mode=adult_mode,
        interaction_topology=plan.interaction_topology,
        scene_function=plan.scene_function,
        tone=plan.tone,
        interiority_level=plan.interiority_level,
        selected_character_ids=request.selected_npc_ids,
        adult_route_eligible=adult_mode is not AdultRenderingMode.OFF,
        route_blocked=False,
        target_provider_model=target_provider_model,
        sequence_plan_sha256=plan.plan_sha256,
    )


def _minimal_behavioral_scene_plan(plan) -> dict[str, object]:
    return {
        "plan_sha256": plan.plan_sha256,
        "selectors": {
            "interaction_topology": plan.interaction_topology.value,
            "scene_function": plan.scene_function.value,
            "tone": plan.tone.value,
            "interiority_level": plan.interiority_level.value,
        },
        "source_claim_ledger": {
            "ledger_sha256": plan.claim_ledger.ledger_sha256,
            "claims": [
                {
                    ...
                    "affected_character_ids": [
                        str(item) for item in value.affected_character_ids
                    ],
                    "rationale": value.rationale,
                }
                for value in plan.claim_ledger.claims
            ],
            "unresolved_questions": list(plan.claim_ledger.unresolved_questions),
            "prohibited_inferences": list(plan.claim_ledger.prohibited_inferences),
        },
        "event_blocks": [
            {
                "block_id": str(block.block_id),
                "actor_ids": [str(value) for value in block.actor_ids],
                "purpose": block.purpose,
                "causal_basis": block.causal_basis,
                "event_advances": list(block.event_advances),
                "resulting_state": block.resulting_state,
                "evidence_ids": [str(value) for value in block.evidence_ids],
                "source_claim_ids": [str(value) for value in block.source_claim_ids],
                ...
                "writer_scaffold": {
                    "viewpoint_character_ids": [
                        str(value)
                        for value in block.writer_scaffold.viewpoint_character_ids
                    ],
                    "motivation_and_subtext": list(
                        block.writer_scaffold.motivation_and_subtext
                    ),
                    "voice_and_interiority": list(
                        block.writer_scaffold.voice_and_interiority
                    ),
                    "physical_and_material_continuity": list(
                        block.writer_scaffold.physical_and_material_continuity
                    ),
                    "transition_obligations": list(
                        block.writer_scaffold.transition_obligations
                    ),
                    "open_realization_space": list(
                        block.writer_scaffold.open_realization_space
                    ),
                },
            }
            for block in plan.event_blocks
        ],
        "causal_runway": {
            "runway_class": plan.runway.runway_class.value,
            "development_obligations": list(plan.runway.development_obligations),
            "continue_beyond_prompt_endpoint": plan.runway.continue_beyond_prompt_endpoint,
            "stop_reason": plan.runway.stop_reason.value,
            "stop_condition": plan.runway.stop_condition,
        },
        "development_atoms": [
            {
                "atom_id": str(value.atom_id),
                "owner_character_id": str(value.owner_character_id),
                "kind": value.kind.value,
                "strength_before": value.strength_before.value,
                "strength_after": value.strength_after.value,
                "summary": value.summary,
                "source_scene_block_ids": [
                    str(item) for item in value.source_scene_block_ids
                ],
                "inference_limit": value.inference_limit,
            }
            for value in plan.development_atoms
        ],
        "selected_character_ids": [str(value) for value in plan.selected_character_ids],
    }
...
def build_deepseek_composer_messages(
    ...
    selection_payload = packet.get("prompt_selection")
    if selection_payload is not None:
        if not isinstance(selection_payload, dict):
            raise ContractValidationError("Composer prompt selection must be an object")
        selection = from_mapping(PromptSelectionRequest, selection_payload)
        compiled = compile_runtime_system_prompt(selection)
        advertised = packet.get("prompt_compilation_receipt")
        if advertised != runtime_prompt_receipt_payload(compiled.receipt):
            raise ContractValidationError("Composer prompt receipt changed after packet assembly")
        user = (
            "The complete authoritative Composer packet follows as canonical JSON:\n"
            + canonical_json(packet)
        )
        system_prompt = compiled.system_prompt
        if packet.get("creator_revision") is not None:
            system_prompt += "\n\n" + _CREATOR_REVISION_SYSTEM_INSTRUCTION
        return (
            DeepSeekMessage("system", system_prompt),
            DeepSeekMessage("user", user),
        )
    system = "\n".join(
        (
            ...
            "Conventional social pleasantries are still factual claims. Do not improvise a prior trip or commute, fatigue, meal, sleep, work or school day, schedule, errand, health condition, familiar preference, or earlier acquaintance merely because such a line sounds welcoming or natural. Use a neutral greeting or a plan-authorized present observation when those facts are absent.",
            "World and continuity facts are closed-world. Broad directions such as orient, tour, explain, host, investigate, or continue do not authorize customary factual fill. Copy concrete facts only from exact source, exact continuity, or exact evidence. This includes left/right and every spatial relation, room location, adjacency, route, distance, object location, possession, schedule, time estimate, routine, procedure, rule, role, history, frequency, prior familiarity, meal, chore, trip, school, work, health, and access detail. If a category is authorized but its concrete value is absent, stay at the category level or omit it; never choose a plausible value. Never infer a converse, inverse, default, exception, or implementation detail absent from evidence. Compare every directional or relational term against continuity before returning.",
            ...
            "Do not return protected_user_id or source-unit realization_segments. source_coverage exclusively binds supplied user material to story segments, and Python derives all protected-user bookkeeping from the exact source ledger. realization_segments are only for advertised current NPC beat authority IDs. In prose, supplied spoken words remain dialogue and supplied physical motion remains action; never expand them into an unsupplied vocalization, reaction, commitment, motive, emotion, or another broader interpretation.",
            ...
            "Every realization_segments entry must contain exactly one authority_id. Do not return owner_id: Python derives the character owner deterministically from that authority. authority_id must be an exact current NPC beat ID from the decision, and the prose must realize that beat's advertised actor. Never use a source_unit_id in realization_segments; source_coverage owns supplied user material. Every required current decision beat needs at least one realization_segments entry naming that exact beat authority_id. Every selected NPC needs at least one of that NPC's beat authorities realized. The kind field is only a general story function from its advertised enum; never put adult channel names such as physiology or narration in kind.",
...
def _compile_v6_python_owners(
    ...
    for value in response.realization_segments:
        if value.authority_id.kind is IdKind.SOURCE_UNIT:
            # source_coverage already binds exact protected-user material to
            # provider story segments.  Provider-authored kind labels for the
            # user are redundant and can falsely broaden source authority.
            continue
