...
class ScriptedJob4FixtureRuntime:
    ...
    def _validator_value(self, _prompt: str) -> ContinuousSemanticValidatorDraftV8:
        harness = self._current()
        if harness._active_validator_label == "scene-1-validator-summary":
            accepted_ids = tuple(
                value.accepted_turn_id for value in harness.accepted_pairs
            )
            return ContinuousSemanticValidatorDraftV8.from_v4(ContinuousSemanticValidatorDraftV4(
