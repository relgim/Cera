...
CONTINUOUS_VALIDATOR_PROMPT_VERSION = "cera.continuous_validator_prompt.v30"
...
For every canonical and diagnostic span, the semantic kind and assertion-owner roles must satisfy the exact closed matrix. Every roles object must contain at least one character in one of its seven arrays; never return an all-empty role ledger. Kind action requires at least one action_owner and no state_owner or speaker. Kind dialogue requires exactly one speaker and no action_owner or state_owner. Kind private_state or consent_or_decision requires at least one state_owner and no action_owner or speaker. Kind narration requires no action_owner, state_owner, or speaker; it must contain at least one active character in affected_ids, addressed_ids, observing_ids, or referenced_ids. Never label a state-owned description as narration. If exact text contains more than one of these semantic kinds, split it at exact codepoint boundaries until every span satisfies exactly one row."""
...
def build_validator_prompt(
    *,
    ...
    request = {
        "schema_version": "cera.continuous_validator_request.v11",
